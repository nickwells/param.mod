...
The advantage of using the New method over using the NewOrDie method is that
you can more easily write a test of your parameter setting code. You put all
the code to generate the PSet in a single method and then call that in a
panic-safe wrapper and test that it doesn't panic. This means that any param
errors will be found during testing while still ensuring that the program
will not run if they are somehow missed.
*/
package paramset
