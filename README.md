...
[![go.dev reference](https://img.shields.io/badge/go.dev-reference-green?logo=go)](https://pkg.go.dev/mod/github.com/nickwells/param.mod/v7)
[![Go Report Card](https://goreportcard.com/badge/github.com/nickwells/param.mod/v7)](https://goreportcard.com/report/github.com/nickwells/param.mod/v7)
...
contains just a `makeParamSet` function. This should take a pointer to a
`Prog` struct (which is unique to the program). The `Prog` struct members
should be set by the parameters (so you don't have any global variables) and
the `makeParamSet` function should call `paramset.New`. This will panic if
there are any problems setting the parameters since `param.NewPSet` will
panic on errors and so can be tested by calling it from within a panic-safe
wrapper.
...
You can specify a terminal parameter (by default `--` but it can be changed
with `param.PSet.SetTerminalParam`) and the remaining parameters will be
