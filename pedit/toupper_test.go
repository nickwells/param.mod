package pedit_test

import (
	"fmt"

	"github.com/nickwells/param.mod/v7/paramset"
	"github.com/nickwells/param.mod/v7/pedit"
	"github.com/nickwells/param.mod/v7/psetter"
)

// ExampleToUpper_Edit demonstrates the behaviour of the ToUpper editor.
//
// Note that the paramset function used here is just to make the example more
// reliable. In production code you would be best to use
// paramset.New(...) which will set the standard helper and panic if
// there's any error. Similarly, in production code, you would call Parse
// with no arguments in which case it will use the arguments given to the
// program (os.Args).
func ExampleToUpper_Edit() {
	ps := paramset.NewNoHelpNoExitNoErrRpt() // use paramset.New()

	var s string

	ps.Add("param",
		psetter.String[string]{
			Value:  &s,
			Editor: pedit.ToUpper{},
		}, "help text",
	)

	ps.Parse([]string{"-param", "Abc"}) // use ps.Parse()
	fmt.Println(s)

	// Output:
	// ABC
}
